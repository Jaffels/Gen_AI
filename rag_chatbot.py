...
def get_document_from_db(doc_id):
    ...
    cursor.execute("SELECT id, name, content, pages, upload_date, file_size, processed FROM documents WHERE id = ?",
                   (doc_id,))
    doc = cursor.fetchone()
    conn.close()
...
def load_session_from_db(session_name):
    ...
    cursor.execute("SELECT document_id FROM session_documents WHERE session_id = ?", (session_id,))
...
with col2:
    ...
    if all_docs:
        ...
        # Display as table with selection checkboxes
        ...
        selected_doc_ids = edited_df[edited_df["Select"]]["ID"].tolist()

        # Option to delete selected documents
        if st.button("Delete Selected Documents"):
            ...
            st.success(f"Deleted {len(selected_doc_ids)} documents.")
            st.rerun()
    else:
...
# Function to process selected documents from database
...
if process_button:
    with st.spinner("Processing documents..."):
        ...
        # Process selected documents from the database
        if 'doc_selection' in st.session_state:
            edited_df = st.session_state.doc_selection
            selected_doc_ids = edited_df[edited_df["Select"]]["ID"].tolist()

            if selected_doc_ids:
                db_text, db_processed_files = process_selected_documents(selected_doc_ids)
                text_to_process += db_text
                newly_processed_files.extend(db_processed_files)
                processed_doc_ids.extend([doc["id"] for doc in db_processed_files])
